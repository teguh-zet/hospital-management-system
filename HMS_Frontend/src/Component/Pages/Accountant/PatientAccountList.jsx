...
function PatientAccountList() {
  ...
  //   });
  // };
  const UpdatePayment = (patientId) => {
    resetData(); // Reset state 'pat' ke nilai awal
    PrivateAxios.get(`patients/` + patientId).then((response) => {
      const patientData = response.data;

      // Siapkan data awal untuk state 'pat'
      const initialPatData = {
        doctorFee: patientData.doctor?.doctorFee || 0,
        // Gunakan optional chaining (?.) untuk mengakses wardCharges dengan aman
        // Jika patientData.ward null, nilainya akan menjadi 0
        wardCharges: patientData.ward?.wardCharges || 0,
      };

      // Panggil getHealthHistory dan teruskan data awal ini
      getHealthHistory(patientId, initialPatData);
    });
  };
