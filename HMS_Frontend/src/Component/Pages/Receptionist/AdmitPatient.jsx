...
function AdmitPatient() {
  ...
  const changePage = (pageNumber = 0, pageSize = 10) => {
    ...
    GetPatientForAdmit(pageNumber, pageSize)
      .then((serverData) => {
        setData({
  ...
  // };
  const getAllWards = () => {
    PrivateAxios.get(`wards/`)
      .then((response) => {
        console.log("Wards received from server:", response.data); // Tambahkan ini untuk debugging
        setWard(response.data);
        toggle(); // Pindahkan toggle ke dalam .then()
      })
      .catch((error) => {
